...
| Can you make a userscript like Duolingo-PRO that is safe or localized? | No, my goal here isn't to create cheats it is just to make learning more accesible and easier on Duolingo like how it used to be a couple years ago.  |
...
| How do I install the Unlimited Hearts userscript? | Install the [Tampermonkey](https://www.tampermonkey.net/) extension and click [this link](https://github.com/apersongithub/Duolingo-Unlimited-Hearts/raw/refs/heads/main/userscript/Duolingo%20Unlimited%20Hearts.user.js). Greasemonkey and other similar extensions may also work. |
...
| How do I know you won't add malicious code to the [extension-version.json](https://github.com/apersongithub/Duolingo-Unlimited-Hearts/blob/main/extension-version.json)?  | This is a good question. Adding a `<script>` element won't do anything but it is possible using an `onerror` tag. In the next version I will make sure tags and elements like that are excluded (hard-coded) so the end user won't have to worry about any external malicious code being added. A beta version will come out soon. |
